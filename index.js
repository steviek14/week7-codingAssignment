...
let agesArray = [3, 9, 23, 64, 2, 8, 28, 93]
   ...
   let firstValue = agesArray[0]; //declare a variable firstValue to find the first element in the agesArray
   let lastIndex = agesArray.length-1; //declare a variable lastIndex to find the last element in agesArray
   let lastValue = agesArray[lastIndex]; // decalre a variable lastValue set to the variable lastIndex which finds the last element in the array
...
let sum = 0 //declare a sum set to 0 - starting our sum count
   for (let i = 0; i <agesArray.length; i++) {
   sum += agesArray[i]
   } // for loop that adds 1 count to the sum and continues to iterate as long as the sum is smaller than the arrays length
...
let totalLetters = 0 //decalre a variable that keeps count of the total letters in the array
   ...
   } // this will iterate over the array as long as the total letters is less than the names array length and it will add one count through each iteration
...
//Part 4:
console.log('Part 4:')
console.log("How do you access the first element if any array?")
...
//Part 5:
console.log("Part 5:")
var namesLengths = [ ] //declare an empty array to later push the results of the loop
for (let i = 0; i < names.length; i++){ //this loop iterates until i is less than the length of the sum of the array names
   namesLengths.push(names[i].length)
} //we are then pushing the result of the for loop to the empty array
...
//Part 6:
console.log("Part 6: ")
let sum2 = 0 //declare a sum set to 0 - starting our sum count
   for (let i = 0; i <namesLengths.length; i++) {
   sum2 += namesLengths[i]
   } //I did this above initally for part 2 but assigned it to namesLength array
...
let function1 = (word, n) => {
   let result = ' ';
   for (let i =0; i < n; i++) //tells the loop to repeat the iteration of the word n number of times
...
// Function to calculate average (as provided in the previous response)
function calculateAverage(numbers) {
   if (numbers.length === 0) {
       return 0;
   }
